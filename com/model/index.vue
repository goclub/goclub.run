<template>
    <div>
                        ...
                        <tr v-for="(row, index) in model.fields">
                            ...
                            <td>
                                <el-select size="mini"
                                           ...
                                           :value="row.goType" filterable style="display: inline-block;"
                                           @input="(v) => changeGoType(v, index)">
                                    <el-option
        ...
        原始数据:
        <el-input autosize type="textarea" :value="JSON.stringify(model,false, '    ')"></el-input>
    </div>
</template>
<script>
...
export default {
	...
	methods: {
		...
		modelData(kind) {
			const vm = this;
			const v = JSON.parse(JSON.stringify(vm.model));
			...
			var c = {
				...
				goType: function (item, prefix) {
					...
					if (type === "custom") {
						item.goTypeCustom = item.goTypeCustom || ''
			...
			if (item.goType === "custom") {
				if (!item.goTypeCustom) {
					if (item.column.endsWith('id')) {
						newValue = "ID" + h.toCamel(item.column.replace(/(_)?id$/, ''))
					} else {
						newValue = vm.model.structName + item.goField
					}
				}
			}
			if (newValue) {
				vm.model.fields[index].goTypeCustom = newValue
			}
			vm.model.fields[index].goType = v
		},
